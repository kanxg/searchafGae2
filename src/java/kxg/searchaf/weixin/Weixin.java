...
public class Weixin {
	...
	private boolean _login() {
		try {
			...
			if (status == HttpStatus.SC_OK) {
				...
				Gson gson = new Gson();
				LoginJson retcode = gson.fromJson(ret, LoginJson.class);
	...
	private int parseFansCount(String text) {
		try {
			...
			Gson gson = new Gson();
			java.lang.reflect.Type type = new com.google.gson.reflect.TypeToken<List<FansCount>>() {
			}.getType();
			List<FansCount> fans = gson.fromJson(json.toString(), type);
	...
	private int parseFans(String text) {
		try {
			...
			Gson gson = new Gson();
			java.lang.reflect.Type type = new com.google.gson.reflect.TypeToken<List<Fan>>() {
			}.getType();
			fans = gson.fromJson(friendlistjson, type);

			// fans=JSON.parseArray(friendlistjson,Fan.class);
			System.out.println("粉丝列表：");

			/**
			 * id:就是后面要用到的fakeid nick_name:昵称 remark_name:备注
			 * group_id:0为未分组,其他的忘了，自己摸索吧
			 */
			for (Fan fan : fans) {
				System.out.println("ID:" + fan.getId() + " nick_name:"
						+ fan.getNick_name() + " remark_name:"
						+ fan.getRemark_name() + " group_id:"
						+ fan.getGroup_id());
			}
			return fans.size();
		...
		try {
			...
			if (this.isLogin) {
				...
				if (status == HttpStatus.SC_OK) {
					...
					try {
						Gson gson = new Gson();
						MsgJson ret = gson.fromJson(text, MsgJson.class);
	...
	public String getUserbyName(String name) {
		...
		for (Fan fan : fans) {
			...
			if (name.equalsIgnoreCase(fan.getNick_name()))
				return fan.getId();
		}
		return null;
	}
	 ...
	 * @date 2013-11-20
	 */
	public boolean sendMsg(String userid, String msgText) {
		try {
			if (!this.isLogin) {
				this._login();
			}
			if (this.isLogin) {
				...
				post.setHeader(
						REFERER_H,
						"https://mp.weixin.qq.com/cgi-bin/singlesendpage?t=message/send&action=index&tofakeid="
								+ userid
								+ "&token="
								+ this.token
								+ "&lang=zh_CN");
				...
				formParams.add(new BasicNameValuePair("content", msgText)); // 说话内容
				formParams.add(new BasicNameValuePair("imgcode", ""));
				formParams.add(new BasicNameValuePair("lang", "zh_CN"));
				formParams.add(new BasicNameValuePair("random", Math.random()
						+ "8"));
				formParams.add(new BasicNameValuePair("tofakeid", userid));
...
//			 wx.client.getHostConfiguration().setProxyHost(
//			 new ProxyHost("cn-proxy.sg.oracle.com", 80));
//			 wx.client.getParams().setParameter("http.useragent",
//			 "Test Client");
//			
//			 HttpHost proxy = new HttpHost("cn-proxy.sg.oracle.com", 80);
//			 wx.httpClient.getParams().setParameter(ConnRoutePNames.DEFAULT_PROXY,
//			 proxy);
			...
			for(int i=1;i<=1;i++){
				wx.sendMsg(wx.getUserbyName("心刚"), "Hello World!");// 像好友列表中的第几个好友发消息，从0开始
			}
