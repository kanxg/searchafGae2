...
<%@ page import="kxg.searchaf.url.zara.SearchZaraJSP" %>
<%@ page import="kxg.searchaf.url.zara.ZaraProduct" %>
<%@ page import="kxg.searchaf.url.zara.ZaraConstant" %>
...
<%
    ...
    if(showdiscount==null){
		showdiscount=String.valueOf(ZaraConstant.warningdiscount);
		firstload=true;
	}
	...
	List<ZaraProduct> matchprolist = null;
	if(!firstload ){
...
%>
    ...
    <form name="frm" action="<%=request.getContextPath()%>/zara/customersearch" method="post" onsubmit="return checkinput()">
      <table>
       <tr>
   ...
   <table width='100%'>  
     ...
     if(matchprolist!=null){
      for (int i = 0; i < matchprolist.size(); i++) {
    	  ZaraProduct product= matchprolist.get(i);
    	  int mod=i % 4 ;
     	  if(mod==0){
       %>
       	 <tr width='100%'>  
