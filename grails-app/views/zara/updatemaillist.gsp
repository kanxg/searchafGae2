...
<%@ page import="kxg.searchaf.url.zara.ZaraMailListJSP" %>
<%@ page import="kxg.searchaf.url.zara.ZaraMailList" %>
...
<%	
	 ...
	 if(mailaddress!=null){
	 	...
	 	if("update".equals(action)){
			...
			ZaraMailListJSP jsp=new ZaraMailListJSP();
			ZaraMailList afMailList1=jsp.loadMail(mailaddress);
			if(afMailList1!=null){
...
<html lang="en">
 ...
 <form id="form1" class="form-horizontal" action="<%=request.getContextPath()%>/zara/updatemaillist" method="post">
...
$( document ).ready(function(){
  ...
   $("#loadbutton").click(function() {
  	  $("#form1").attr("action", "<%=request.getContextPath()%>/zara/updatemaillist?action=load");
   });
  ...
   $("#updatebutton").click(function() {
	...
	if(Number(tem4)<0.01 || Number(tem2)>1){
		...
		return false;
   		...
   		$("#form1").attr("action", "<%=request.getContextPath()%>/zara/updatemaillist?action=update");
   });
